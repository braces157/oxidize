...
//! Permanent test target verifying parity review defects R01 through R12.
...
#[test]
fn submodule_update_must_reject_dirty_submodule_worktree() {
    let main_repo = repo();
    let r = main_repo.path();
    let gd = r.join(".git");
    let sub_repo = repo();
    let sub_r = sub_repo.path();

    let sub_url = sub_r.to_str().unwrap().replace('\\', "/");
    let gitmodules = format!(
        "[submodule \"mysub\"]\n\tpath = mysub\n\turl = {}\n\tbranch = main\n",
        sub_url
    );
    fs::write(r.join(".gitmodules"), &gitmodules).unwrap();
    ops::submodule_init(r, &gd, "mysub").unwrap();
    ops::submodule_update(r, &gd, "mysub").unwrap();
    assert!(r.join("mysub").join("f.txt").exists());

    // Introduce dirty modification in submodule worktree
    fs::write(
        r.join("mysub").join("f.txt"),
        "UNCOMMITTED SUBMODULE WORK\n",
    )
    .unwrap();

    let result = ops::submodule_update(r, &gd, "mysub");
    assert!(
        result.is_err(),
        "submodule_update should reject dirty worktree"
    );
    assert_eq!(
        fs::read_to_string(r.join("mysub").join("f.txt")).unwrap(),
        "UNCOMMITTED SUBMODULE WORK\n",
        "submodule worktree modification was overwritten"
    );
}

#[test]
fn custom_patch_to_non_head_must_fail_safely() {
    let t = repo();
    let r = t.path();
    let gd = r.join(".git");
    let base: ObjectId = git(r, &["rev-parse", "HEAD"]).parse().unwrap();
    fs::write(r.join("f.txt"), "head content\n").unwrap();
    git(r, &["commit", "-am", "second"]);
    let head_before: ObjectId = git(r, &["rev-parse", "HEAD"]).parse().unwrap();

    let mut basket = oxidize_diff::CustomPatchBasket::new();
    let hunk = compute_structured_diff("one\n", "one\npatch\n", 3).remove(0);
    basket.add_hunk("f.txt", hunk);

    let res = ops::apply_custom_patch_to_commit(r, &gd, &basket, base, false);
    assert!(
        res.is_err(),
        "non-HEAD custom patch application must be rejected"
    );
    let head_after: ObjectId = git(r, &["rev-parse", "HEAD"]).parse().unwrap();
    assert_eq!(head_before, head_after, "HEAD commit changed unexpectedly");
}

#[test]
fn action_dispatch_tag_and_branch_with_decoy_same_name() {
    let t = repo();
    let r = t.path();
    git(r, &["branch", "shared-name"]);
    git(r, &["tag", "shared-name"]);

    let mut app = App::new();
    app.load_repository(&r.join(".git")).unwrap();

    // 1. In Tags tab, delete action must prompt for DeleteTag
    app.active_panel = Panel::Branches;
    app.branches_tab = BranchesTab::Tags;
    app.tags_selected = app
        .tags
        .iter()
        .position(|tag| tag.name == "shared-name")
        .unwrap();
    app.prompt_delete_selected_tag();
    match &app.active_modal {
        ActiveModal::Confirm {
        ...
        other => panic!("expected DeleteTag confirmation, got {other:?}"),
    }

    // 2. In Local branches tab, delete action must prompt for DeleteBranch
    app.branches_tab = BranchesTab::Local;
    app.branches_selected = app
        .branches
        .iter()
        .position(|b| b.name == "shared-name")
        .unwrap();
