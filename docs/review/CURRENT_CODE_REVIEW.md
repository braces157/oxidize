...
12. **Staged formatting semantics have been corrected.** `fmt --staged` now formats/checks staged blob snapshots and updates the index without modifying working-tree files, preserving partial staging. Hook installation remains a separate Git hook integration and should not be treated as proof that Oxidize's native commit path executes hooks internally.
